...
void CAxisWidget::onClearState()
{
	...
	if (ok) {
		m_pGTSControllerWidget->showLog(
			QStringLiteral("轴%1 状态已清除").arg(m_iAxisIndex), Qt::darkGreen);
	}
	else {
		m_pGTSControllerWidget->showLog(
			QStringLiteral("轴%1 状态清除失败").arg(m_iAxisIndex), Qt::red);
	}
	onUpdateAxisInfo();
}
...
void CAxisWidget::onServoOn()
{
	...
	if (ok) {
		m_pGTSControllerWidget->showLog(
			QStringLiteral("轴%1 使能成功").arg(m_iAxisIndex), Qt::darkGreen);
	}
	else {
		m_pGTSControllerWidget->showLog(
			QStringLiteral("轴%1 使能失败").arg(m_iAxisIndex), Qt::red);
	}
	onUpdateAxisInfo();
}
...
void CAxisWidget::onServoOff()
{
	...
	if (ok) {
		m_pGTSControllerWidget->showLog(
			QStringLiteral("轴%1 已禁止").arg(m_iAxisIndex), Qt::darkGreen);
	}
	else {
		m_pGTSControllerWidget->showLog(
			QStringLiteral("轴%1 禁止失败").arg(m_iAxisIndex), Qt::red);
	}
	onUpdateAxisInfo();
}
...
void CAxisWidget::onClearPos()
{
	...
	if (ok) {
		m_pGTSControllerWidget->showLog(
			QStringLiteral("轴%1 位置已清零").arg(m_iAxisIndex), Qt::darkGreen);
	}
	else {
		m_pGTSControllerWidget->showLog(
			QStringLiteral("轴%1 位置清零失败").arg(m_iAxisIndex), Qt::red);
	}
	onUpdateAxisInfo();
}
...
void CAxisWidget::onSmoothStop()
{
	...
	if (ok) {
		m_pGTSControllerWidget->showLog(
			QStringLiteral("轴%1 减速停止").arg(m_iAxisIndex), Qt::darkGreen);
	}
	else {
		m_pGTSControllerWidget->showLog(
			QStringLiteral("轴%1 减速停止失败").arg(m_iAxisIndex), Qt::red);
	}
}
...
void CAxisWidget::onEStop()
{
	...
	if (ok) {
		m_pGTSControllerWidget->showLog(
			QStringLiteral("轴%1 急停已触发").arg(m_iAxisIndex), Qt::darkGreen);
	}
	else {
		m_pGTSControllerWidget->showLog(
			QStringLiteral("轴%1 急停失败").arg(m_iAxisIndex), Qt::red);
	}
}
...
void CAxisWidget::onActMotion()
{
	...
	m_pGTSControllerWidget->showLog(
		QStringLiteral("轴%1 运动触发（待实现）").arg(m_iAxisIndex), Qt::blue);
}
